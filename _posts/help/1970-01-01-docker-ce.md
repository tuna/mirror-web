...
<code class="language-bash" id="deb-amd64-content">sudo add-apt-repository \
   "deb [arch=amd64] https://{{ site.host }}/docker-ce/linux/{{deb_release}} \
   $(lsb_release -cs) \
   stable"</code>
</pre>
{% endraw %}
...
<p></p>
<pre>
<code id="deb-arm-content">echo "deb [arch=armhf] https://{{ site.host }}/docker-ce/linux/{{deb_release}} \
...
```bash
sudo sed -i 's+download.docker.com+{{ site.host }}/docker-ce+' /etc/yum.repos.d/docker-ce.repo
```
