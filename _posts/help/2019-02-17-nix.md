...
- 单用户安装
    ...
    ```console
    $ sh <(curl https://{{ site.host }}/nix/latest/install)
    ```
- 多用户安装：

    ```console
    $ sh <(curl https://{{ site.host }}/nix/latest/install) --daemon
    ```

如果需要，可以在[文件列表中手动挑选需要的版本](https://{{ site.host }}/nix)。

### NixOS

NixOS 的安装镜像和 OVF 虚拟机镜像可以在镜像站首页“下载链接”处或 <https://{{ site.host }}/nixos-images/> 下载。

### Nixpkgs binary cache

Binary cache 的镜像位于 <https://{{ site.host }}/nix-channels/store>，与 channel 同步更新。
...
- 单独安装的 Nix：编辑配置文件添加或修改如下项（通常系统配置在 `/etc/nix/nix.conf`，用户配置在 `~/.config/nix/nix.conf`）：
    ...
    ```plain
    substituters = https://{{ site.host }}/nix-channels/store https://cache.nixos.org/
    ```

- NixOS 和 nix-darwin 使用如下配置：

    ```nix
    nix.binaryCaches = [ "https://{{ site.host }}/nix-channels/store" "https://cache.nixos.org/" ];
    ```

- NixOS 19.09 及以后版本和 nix-darwin 中系统会自动配置备选源站，也可用如下配置：

    ```nix
    nix.binaryCaches = [ "https://{{ site.host }}/nix-channels/store" ];
    ```

### Nixpkgs channel

Channel 的镜像位于 <https://{{ site.host }}/nix-channels/> 目录下，使用方式与 <https://nixos.org/channels/> 相同。

单独安装的 Nix 替换 `nixpkgs-unstable` 命令如下：

```console
$ nix-channel --add https://{{ site.host }}/nix-channels/nixpkgs-unstable
$ nix-channel --update
```
...
```console
# nix-channel --add https://{{ site.host }}/nix-channels/nixos-19.09
# nix-channel --update
```
