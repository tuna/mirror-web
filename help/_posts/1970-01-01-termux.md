...
sed -i 's@^\(deb.*root stable\)$@#\1\ndeb https://{{ site.hostname }}/termux/game-packages-24 root stable@' $PREFIX/etc/apt/sources.list.d/root.list
...
```
# The termux repository mirror from TUNA:
deb https://{{ site.hostname }}/termux/science-packages-24 root stable
```
