...
const options = Object.fromEntries(globalOptions.force_redirect_help_mirrors.map(m => [
    m, {'url': "/help/" + m + "/"}
]));
...
const stringifyTime = (ts) => {
  const date = new Date(ts * 1000);
	...
	if (date.getFullYear() > 2000) {
		str = `${('000'+date.getFullYear()).slice(-4)}-${('0'+(date.getMonth()+1)).slice(-2)}-${('0'+date.getDate()).slice(-2)}` +
